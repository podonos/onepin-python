...
- The backend (donut-be) and the SDK version are **fully independent**. The backend prod
 ...
 repository_dispatch ─────┤   release-please ──cuts──▶ tag vX.Y.Z ───┤                                        │
   (environment=dev,      │                                          ▼                                        │
    smoke-check only)     │                                  publish.yml                                      │
                          ...
                          ┌──────────────────────────────── PyPI lane (prod gate) ──────────────────────────┐
 backend PROD deploy      │                                                                                  │
 (deploy-prod.yml, 4b) ───┼─▶ repository_dispatch[api-spec-updated] {sha=S, spec_version, environment: prod} │
                          │                                  │                                               │
                          │                                  ▼                                               │
                          │                          promote-prod.yml                                        │
...
- Real backend **prod** deploy (`deploy-prod.yml` 4b) → `repository_dispatch{environment:prod}`
