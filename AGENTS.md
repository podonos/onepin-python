...
| `.github/workflows/regen.yml` | Self-generates the SDK from the donut-be spec | YES |
...
Files under `src/onepin/` (excluding `_cli/`) are generated by [Fern](https://buildwithfern.com/) from the donut-be OpenAPI spec, which carries the SDK shaping (`x-fern-*` + `servers`). **Do not patch them locally** — the next regen will overwrite your changes.

**If you need to change generated behavior, change the spec in donut-be.** Generation runs *in this repo*: `.github/workflows/regen.yml` fetches the shaped spec at a donut-be commit and runs `fern generate --local` (config in `fern/`), then opens a regen PR.
...
- OpenAPI spec edits (including the SDK shaping) live in donut-be, not here.
- Documentation site content lives in onepin-sdks.
