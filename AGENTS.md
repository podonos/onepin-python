...
Files under `src/onepin/` (excluding `_cli/`) are auto-generated by [Fern](https://buildwithfern.com/) from the upstream OpenAPI spec. **Do not patch them locally** — the next regen will overwrite your changes.

**If you need to change generated behavior, change the OpenAPI spec.** Then re-run the generator. The Fern auto-PR will land in this repo with the updated SDK.

After every Fern regen, `scripts/post_fern.sh` must run to restore PEP 561 `py.typed` markers. The CI publish pipeline runs this automatically; local regens require running it by hand.

`.fernignore` is not currently present. If a patch on a generated file becomes unavoidable, add the file to `.fernignore` with a comment explaining why, and document the patch in this file under a new "Active patches" section.
...
- OpenAPI spec edits live upstream, not here.
- Fern generator configuration lives upstream.
